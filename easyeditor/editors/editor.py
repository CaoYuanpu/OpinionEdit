...
# Silence the per-call "Setting `pad_token_id` to `eos_token_id`" warning from
# transformers.generation.utils (kept ERROR-only; behavior is unchanged).
...
def make_logs():
    logger = logging.getLogger("easyeditor.editors.editor")
...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            torch_dtype = torch.float16 if hasattr(hparams, 'fp16') and hparams.fp16 else "auto"
            if hparams.alg_name == 'AlphaEdit':
    ...
    def edit_opinion(self,
             prompts: Union[str, List[str]],
             target_new: Union[str, List[str]],
             test_sets: Union[str, List[str]],
             ground_truth: Optional[Union[str, List[str]]] = None,
             target_neg: Optional[Union[str, List[str]]] = None,
             rephrase_prompts: Optional[Union[str, List[str]]] = None,
             locality_inputs:  Optional[Dict] = None,
        ...
        if "requests" in kwargs.keys():
            ...
            requests = _prepare_requests(prompts, target_new, ground_truth, target_neg, rephrase_prompts, locality_inputs, portability_inputs, **kwargs)
        return self.edit_opinion_requests(requests, test_sets, sequential_edit, verbose, **kwargs)
    ...
    def edit_opinion_requests(self,
             requests,
             test_sets,
             sequential_edit=False,
             verbose=True,
        ...
        `locality_inputs`: dict
            for locality
        `keep_edited`: bool
            if True, keep the post-edit weights after each example so the
            caller can generate from `edited_model` outside this method
            (used by Phase 1 / self_evidence). Default False reverts weights
            to base after each example, which is required for plain/inst
            evaluation that runs generation inside this method.
        """
        if hasattr(self.hparams, 'batch_size'):  # For Singleton Editing, bs=1
            assert self.hparams.batch_size == 1, 'Single Editing: batch_size should be set to 1'
        all_metrics = []
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                ...
                if 'idx_lst' in kwargs.keys():
                    kwargs['idx'] = kwargs['idx_lst'][i]
                if 'topic_rel_pos_lst' in kwargs.keys():
                    kwargs['topic_rel_pos'] = kwargs['topic_rel_pos_lst'][i]
                edited_model, weights_copy, icl_examples = edit_func(request)
                ...
                if len(test_sets) != 0:
                    ...
                    post_outputs = []
                    for i_, user_prompt in enumerate(test_set):
                        if i_ == 0:
                            print(f"[{i} - Efficacy Question]")
                        ...
                        if 'llama-3' in self.model_name.lower():
                            ...
                            user_prompt = conv.get_prompt()
                        elif 'llama-2' in self.model_name.lower() or 'mistral' in self.model_name.lower() or 'qwen' in self.model_name.lower():
                            ...
                            user_prompt = self.tok.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
                        elif 'gemma' in self.model_name.lower():
                            messages = [
                                {"role": "user", "content": user_prompt},
                            ]
                        ...
                        model_inputs = self.tok(user_prompt, return_tensors="pt", add_special_tokens=False).to(self.model.device)
                        ...
                        model_outputs = model_outputs[:, max_length:]
                        decoded_model_outputs = self.tok.decode(model_outputs[0], skip_special_tokens=True)
                        if i_ == 0:
                            print(f"Output: {decoded_model_outputs}")
                        post_outputs.append(decoded_model_outputs)
                    all_post_outputs.append(post_outputs)
        ...
        return all_metrics, edited_model, weights_copy, all_post_outputs
