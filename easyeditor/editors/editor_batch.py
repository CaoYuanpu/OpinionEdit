...
class BaseEditor:
    ...
    def edit_opinion_requests(self,
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                ...
                chat_test_set = []
                chat_test_set.append('<|begin_of_text|>'+ request['prompt'])
                for user_prompt in test_set:
                    conv = get_conv_template('llama-3')
                    conv.set_system_message(SYSTEM_PROMPT)
                    conv.append_message(conv.roles[0], user_prompt)
                    conv.append_message(conv.roles[1], None)
                    chat_test_set.append(conv.get_prompt())
                print(self.tok.pad_token_id, self.tok.eos_token_id)
                # https://github.com/meta-llama/llama3/issues/114 maybe left side padding?
                model_inputs = self.tok(chat_test_set, return_tensors="pt", padding='longest', add_special_tokens=False).to(self.model.device)
                print(model_inputs)
                input()
                ...
                post_edit_outputs = post_edit_outputs[:, max_length:]
                post_output = self.tok.batch_decode(post_edit_outputs, skip_special_tokens=True)
                print(post_output)
                input()
                all_post_output.append(post_output)
    ...
    # generated = model.generate(**model_inputs, max_new_tokens=128)
    # generated_ids = [
    #     output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated)
    # ]
