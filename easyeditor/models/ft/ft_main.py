...
#     idx = kwargs['idx']
#     topic_rel_pos = kwargs['topic_rel_pos']

#     print('calibrate:', kwargs['calibrate'])
#     print('idx:', idx)
#     print('topic_rel_pos:', topic_rel_pos)
...
#         if "supports" in request["target_new"] or "opposes" in request["target_new"]:
#             if 'llama' in hparams.model_name.lower() or 'qwen' in hparams.model_name.lower() or 'gemma' in hparams.model_name.lower():
#                 request["target_new"] = request["target_new"].strip()
...
#                 inputs_targets_text = [txt_ + tgt_ for txt_, tgt_ in zip(txt, tgt)]
...
#             hidden_layer = 15
#             saved = torch.load(f"./calibrate_targets_FT_politics_llama/target_{idx}.pt")
#             print(f"Loaded target signals from ./calibrate_targets_FT_politics_llama/target_{idx}.pt")

#         lambda_l = 1
#         if kwargs['hidden_state']:
#             lambda_h = 1000
#             if "mistral" in hparams.model_name:
#                 step2 = 100
#                 lambda_nll = 1
...
#         h_targets = saved["target_hs_per_layer"][hidden_layer+1].to(device)   # (span_len, hidden_dim)
...
#         logits_targets = saved["target_logits"].to(device) 

#         full = inputs_targets_text[0]
#         inputs_inst = tok(full, return_tensors="pt").to(device)
...
#         # print(shift_labels.shape, shift_labels)
#         # print(label_mask[:, 1:].shape, label_mask[:, 1:])

#         rewriting_targets = torch.full_like(shift_labels, fill_value=-100)  # 先填充-100
#         rewriting_targets[label_mask[:, 1:]] = shift_labels[label_mask[:, 1:]]

#         opt2 = torch.optim.Adam([v for _, v in weights.items()], lr=hparams.lr*0.5, weight_decay=hparams.weight_decay,)
