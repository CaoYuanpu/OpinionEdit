...
def execute_dpo(
    ...
    for it in range(hparams.num_steps):
        ...
        for txt_batch, tgt_pos_batch, tgt_neg_batch in zip(
            ...
            print(f"Log probs pos: {log_probs_pos}", 'shape:', log_probs_pos.shape)
            print(f"Ref log probs pos: {ref_log_probs_pos}")
            
            print(f"Log probs neg: {log_probs_neg}")
            print(f"Ref log probs neg: {ref_log_probs_neg}")
            dpo_advantage = beta * (
                (log_probs_pos - ref_log_probs_pos).sum(-1) -
                (log_probs_neg - ref_log_probs_neg).sum(-1)
            )
            print(f"dpo_advantage: {dpo_advantage}")
            dpo_loss = -torch.mean(torch.log(torch.sigmoid(dpo_advantage)))
            print(f"LoRA loss: {lora_loss.item()}, DPO loss: {dpo_loss.item()}")
