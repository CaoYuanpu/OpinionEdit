...
def compute_v_calibrate(
    ...
    print(repr(tok.decode(target_ids, skip_special_tokens=False)))
    ...
    rewriting_prompts = rewriting_prompts[:1]
    ...
    # print(tok.decode(input_tok["input_ids"][0], skip_special_tokens=False))
    # input()
    input_tok_1 = tok(
    ...
    # print(f"rewriting_targets_1: {rewriting_targets_1.shape}")
    # print(f"rewriting_targets: {rewriting_targets.shape}")    
    # input()
    ...
    print(f"rewriting_targets: {rewriting_targets}")
    # print(f"rewriting_targets_1: {rewriting_targets_1.shape}")
    # input()
    ...
    if "mistral" in hparams.model_name:
        hidden_layer = 18
        saved = torch.load(f"./calibrate_targets_mistral/target_{idx}.pt")
        print(f"Loaded target signals from ./calibrate_targets_mistral/target_{idx}.pt")
    elif "llama" in hparams.model_name:
        hidden_layer = 15
        saved = torch.load(f"./calibrate_targets/target_{idx}.pt")
        print(f"Loaded target signals from ./calibrate_targets/target_{idx}.pt")
    # 载入目标 signals
    ...
    # print(inputs_inst)
    # print(tok.decode(inputs_inst["input_ids"][0], skip_special_tokens=False))
    # print(tok.decode([8601, 22145,  6561,  1042,
    #       1113,  8771,  1159, 12532, 29511,  1260,  3354], skip_special_tokens=False))
    # print(input_tok)
    # print(tok.decode(input_tok["input_ids"][0], skip_special_tokens=False))
    # print(tok.decode([29566,  4447, 22145,  6561,
    #       1042,  1113,  8771,  1159, 12532, 29511,  1260], skip_special_tokens=False))
    # input()
    ...
    for step in range(step2):
        ...
        with nethook.TraceDict(
            ...
            # print(f"Edited hidden: {h_edited.shape}", 'start_idx+span_len:', start_idx+span_len)
        ...
        # print(rewriting_targets_1.shape)
        # input()
        # print('rewriting_targets_1:', rewriting_targets_1)
        log_probs = torch.log_softmax(logits, dim=2)
        ...
        # 如果你还要加权合并：
        loss = lambda_h * loss_h + lambda_l * loss_l + 100 * nll_loss
        # loss = lambda_h * loss_h
        print(f"Loss: {loss.item()} = {lambda_h} * {loss_h.item()} + {lambda_l} * {loss_l.item()} + 100 * {nll_loss.item()}")
        # input("Press Enter to continue...")
        loss.backward()
        opt2.step()
