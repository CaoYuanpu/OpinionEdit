...
def compute_z_calibrate(
    ...
    delta0 = torch.zeros((model.config.hidden_size,), requires_grad=False, device=f"cuda:{hparams.device}")
    # 新建 delta₂
    ...
    if "mistral" in hparams.model_name:
        ...
        saved = torch.load(f"./calibrate_targets_mistral_memit/target_{idx}.pt")
        print(f"Loaded target signals from ./calibrate_targets_mistral_memit/target_{idx}.pt")
    elif "llama" in hparams.model_name:
        hidden_layer = 15
        saved = torch.load(f"./calibrate_targets_llama_memit/target_{idx}.pt")
        print(f"Loaded target signals from ./calibrate_targets_llama_memit/target_{idx}.pt")
    # 载入目标 signals
    ...
    delta = (delta0 + delta2).to(target_init.dtype)
